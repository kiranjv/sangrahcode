 ...
 */
public class InVoiceScreen_OLD extends javax.swing.JPanel {
	...
	public static String[] producttable_headder = { "S NO.", "Bar Code",
			"Product Name", "Unit Price", "Qty", "Discount(%)", "VAT(%)",
			"Net Price" };
	...
	public static void main(String[] args) {
		...
		InVoiceScreen_OLD screen = new InVoiceScreen_OLD();
	...
	private void initGUI() {
		try {
			...
			barcode_panel.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.LOWERED));

			// initialize bar code textfield
			barcode_field = new JTextField();
			barcode_panel.add(barcode_field, new GridBagConstraints(1, 1, 1, 1,
			...
			searchproduct_button.setFont(new Font("Arial",
			...
			ItemsPanel_Box.setBorder(BorderFactory.createEtchedBorder(
					new java.awt.Color(255, 255, 128), null));
			...
			table_invoice.setBorder(new SoftBevelBorder(BevelBorder.LOWERED,
					null, null, null, null));
			...
			tableheaderpanel.setBorder(BorderFactory
			...
			label_totalamount.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.RAISED, null,
			...
			label_discount.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.RAISED, null,
							new java.awt.Color(192, 192, 192)));
			...
			label_grandtotal.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.RAISED, null,
							new java.awt.Color(192, 192, 192)));
			...
			totalamount_lable.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.RAISED, null,
							new java.awt.Color(192, 192, 192)));
			...
			discountamount_lable.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.RAISED, null,
							new java.awt.Color(192, 192, 192)));
			...
			grandtotal_lable.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.RAISED, null,
							new java.awt.Color(192, 192, 192)));
			...
			RoyeltyPanel_Box.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.LOWERED));
			RoyeltyPanel_Box.setLayout(null);
			...
			label_royaltypoint_information
					.setText("Royalty points information");
	...
	private void setActionKeyMapping() {
		...
		keyMap.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_M,
				java.awt.Event.ALT_MASK), "action_save");
		...
		keyMap.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_UP,
				java.awt.Event.ALT_MASK), "action_up");

		keyMap.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_DOWN,
				java.awt.Event.ALT_MASK), "action_down");

		keyMap.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_D,
				java.awt.Event.CTRL_MASK), "action_delete");

		keyMap.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_Q,
				java.awt.Event.ALT_MASK), "action_select");

		keyMap.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_B,
				java.awt.Event.ALT_MASK), "escape_action");

		SwingUtilities.replaceUIActionMap(mainPanel, actionMap);
		SwingUtilities.replaceUIInputMap(mainPanel,
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (barcode.length() > 0) {
				...
				List<VtigerProducts> products = DBLocalHelper.readRecord(
						modelname, col_name, barcode);
				if (products.size() == 0) {
					JOptionPane.showMessageDialog(null, "No product available");
				} else {
					int exist_rownum = isProductAlreadyInTable(products.get(0),
							barcode);
					if (exist_rownum == -1) {
	...
	private int isProductAlreadyInTable(VtigerProducts product, String barcode) {
		...
		if (rows_num != 0) {
			for (int i = 0; i < rows_num; i++) {
				String row_barcode = (String) table_invoiceModel.getValueAt(i,
						barcode_colnum);
				if (row_barcode.equalsIgnoreCase(barcode)) {
					System.out
							.println("row_barcode: " + row_barcode
									+ " barcode:" + barcode
									+ " exist row number: " + i);
					exist_rownum = i;
					return exist_rownum;
	...
	private void updateProductTable(List<VtigerProducts> products, int rownumber) {
		String prev_qty = (String) table_invoiceModel.getValueAt(rownumber,
				qty_index);
		...
		if (rowData.length != 0) {
			CELLUPDATE = true;
			// Update the row
			for (int i = 1; i < table_invoiceModel.getColumnCount(); i++) {
				table_invoiceModel.setValueAt(rowData[i], rownumber, i);
			}
		}
		CELLUPDATE = false;
	}
	 ...
	 * @return
	 */
	private String[] calculateInvoiceData(List<VtigerProducts> products,
			String quatity) {
		String rowData[] = null;
		for (Iterator iterator = products.iterator(); iterator.hasNext();) {
			VtigerProducts vtigerProduct = (VtigerProducts) iterator.next();
			String[] invoice = { vtigerProduct.getProductNo(),
					vtigerProduct.getBarcode(), vtigerProduct.getProductname(),
					vtigerProduct.getUnitPrice(), vtigerProduct.getDiscount(),
					vtigerProduct.getVat() };
	...
	private String[] calculateInvoiceData(String[] invoice, String quantity) {
		...
		if (discount != 0.0) {
			discount_amount = Util.calculate_discount(discount_unitprice,
					discount);
		...
		// calc grand total ammount
		float grand_total = (int) Math.round(discount_unitprice + vat_amount);
		rowData[7] = String.valueOf(grand_total);

		// update total amount field
		float total_amount_all = unit_price;
		totalamount_lable.setText(String.valueOf(Util
				.Round(total_amount_all, 2)));

		// update discount ammount field
		float total_discount_all = Float.parseFloat(discountamount_lable
				.getText()) + discount_amount;
		discountamount_lable.setText(String.valueOf(total_discount_all));

		// update grand total amount
		float grandtotal_all = grand_total;
			...
			public void tableChanged(TableModelEvent tme) {
				if (tme.getType() == TableModelEvent.UPDATE) {
					if (tme.getColumn() == qty_index && !CELLUPDATE) {
						String quatity = (String) table_invoiceModel
								.getValueAt(tme.getFirstRow(), tme.getColumn());
						...
						String last_quantity = (String) table_invoiceModel.getValueAt(updatedrow, 4);
						String rowData[] = calculateUpdateInvoiceData(product, String.valueOf(quatity));
						if (rowData.length != 0) {
							CELLUPDATE = true;
							// Update the row
							table_invoiceModel.setValueAt(rowData[1], updatedrow, 1);
							table_invoiceModel.setValueAt(rowData[2], updatedrow, 2);
							table_invoiceModel.setValueAt(rowData[3], updatedrow, 3);
							table_invoiceModel.setValueAt(rowData[4], updatedrow, 4);
							table_invoiceModel.setValueAt(rowData[5], updatedrow, 5);
							table_invoiceModel.setValueAt(rowData[6], updatedrow, 6);
							table_invoiceModel.setValueAt(rowData[7], updatedrow, 7);
	...
	private String[] calculateUpdateInvoiceData(String[] invoice, String quantity) {
		String rowData[] = new String[table_invoiceModel.getColumnCount()];
		rowData[0] = String.valueOf(table_invoiceModel.getRowCount() + 1);
		rowData[1] = invoice[1]; // vtigerProducts.getBarcode();
		rowData[2] = invoice[2]; // vtigerProducts.getProductname();
		if(Integer.parseInt(quantity) > 2) {
		float current_unitprice = Float.parseFloat(invoice[3]);
		
		float base_unitprice = current_unitprice / previous_quantity;
		rowData[3] = String.valueOf(base_unitprice); // vtigerProducts.getUnitPrice();
		}
		else
		{
			rowData[3] = invoice[3];
