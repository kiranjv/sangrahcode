	...
	private HashMap<String, Object> prepareConsolidateData(String[][] tabledata, float invoice_totalamount, int invoice_totalitemsdiscount,
		...
		for (int i = 0; i < tabledata.length; i++) {
			...
			for (int j = 0; j < invoice.length; j++) {
				product.put("barcodeid", invoice[InVoiceScreen.barcode_index]);
				product.put("productName", invoice[InVoiceScreen.productname_index]);
				product.put("mrp", invoice[InVoiceScreen.unitprice_index]);
				product.put("qty", invoice[InVoiceScreen.qty_index]);
				product.put("discount", invoice[InVoiceScreen.discount_index]);
				String itemdiscount = Util.calcItemDiscount(invoice[InVoiceScreen.unitprice_index], invoice[InVoiceScreen.qty_index],
							invoice[InVoiceScreen.discount_index]);
				product.put("itemdiscount", itemdiscount);
				product.put("vat", invoice[InVoiceScreen.vat_index]);
