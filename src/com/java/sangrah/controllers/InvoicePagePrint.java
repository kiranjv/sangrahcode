...
public class InvoicePagePrint implements Printable {
	...
	public int print(Graphics g, PageFormat pageFormat, int page) {
		...
		for (int i = 0; i < productdata_list.size(); i++) {
			...
			g2d.setFont(normal);
			int sno = i + 1;
			String bcode = product.get("barcodeid");
			String pname = product.get("productName");
			String quantity = product.get("qty");
			String unitprice = product.get("mrp");
			String discount = product.get("discount");
			String vat = product.get("vat");

			float price = Float.parseFloat(product.get("mrp"))
						+ (Float.parseFloat(product.get("mrp")) * (Float.parseFloat(product.get("vat")) / 100));
